    ...
    public async Task<CustomersListViewModel> GetCutomerByPaginationAsync(CustomerPaginationViewModel model)
    {
        ...
        if(model.TimeLog == "Custom Date")
        {
            if (model.CustomFromDate != null && model.CustomToDate != null)
            {
                ...
                // DateTime startDate = new DateTime(model.CustomFromDate.Value.Year, model.CustomFromDate.Value.Month, model.CustomFromDate.Value.Day);
                // DateTime endDate = new DateTime(model.CustomToDate.Value.Year, model.CustomToDate.Value.Month, model.CustomToDate.Value.Day);
